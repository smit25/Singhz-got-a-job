...
> "No active applications to follow up on. Apply to some roles first with `/singhz-got-a-job` and come back when they're aging."
...
- Suggest the user run `/singhz-got-a-job contacto {company}` to find the right person first
...
> - Trying a different contact via `/singhz-got-a-job contacto`
...
**To:** {email or "No contact found — run `/singhz-got-a-job contacto` first"}
