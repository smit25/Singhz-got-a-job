...
8. Select top 3-4 projects most relevant to the role
...
Use the template at `cv-template.html`. Replace `{{...}}` placeholders with tailored content:
...
| `{{SUMMARY_TEXT}}` | Tailored summary with JD keywords |
...
Update tracker if the role is already tracked: change PDF from ❌ to ✅.
