...
- `workday`: `jobPostings[]` (varies by tenant) → `title`, `externalPath` or URL built from host
...
7.5. **Verify liveness of Level 3 WebSearch results** — BEFORE adding to pipeline:
   ...
   WebSearch results can be stale (Google caches results for weeks or months). To avoid evaluating expired offers, verify with Playwright every new URL from Level 3. Levels 1 and 2 are inherently real-time and do not require this check.
...
→ Run /singhz-got-a-job pipeline to evaluate the new offers.
```
...
Fallback: if you only have the direct ATS URL, navigate first to the company's website and find their corporate careers page. Use the direct ATS URL only if the company has no corporate careers page.
